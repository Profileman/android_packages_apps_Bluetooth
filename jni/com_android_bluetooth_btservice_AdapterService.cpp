...
static bool initNative(JNIEnv* env, jobject obj) {
	...
	//pipo rom bluetooth.default.so is older (androi 4.2.x?)
	//and does not expose map_client if... so make sure we never call those functions!
	#if 1
	ALOGV("pipo map_client if hack activated, not calling get_profile_interface(BT_PROFILE_MAP_CLIENT_ID)!");
